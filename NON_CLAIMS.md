...
1. **Not a field-level breakthrough.**  
   Consistent outperformance of current SOTA sequential integer sorters (ska_sort-class, IPS⁴o sequential, advanced pdqsort variants, modern LSD/MSD radix) across a broad reproducible suite is **not claimed**. ska_sort remains stronger on pure high-entropy data (best pure HE residual ~1.02–1.09× isolated / ~1.11× full-menu vs ska). Residual floors remain on adversarial and sparse patterns versus a highly engineered pdqsort.
...
3. **Residual floors are acknowledged (pure residual freeze, n=1e6).**  
   ...
   |---------|---------------:|--------|
   | adversarial_pivot | **~1.81×** | pure residual floor accepted |
   | db_pk_sparse | **~2.56×** | pure residual floor accepted |
   | equal_heavy | **~0.99×** | near parity |
...
6. **EXTERNAL-clean residual path.**  
   Visible probe metrics only. No χ. No hidden irreversible learning state in the frozen residual menu.
