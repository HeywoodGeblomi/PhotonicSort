#pragma once
/*
 * residual_few_wide_i64 — FEW_WIDE pure residual (v2.4.1 + PO5 dense gate)
 * Dense ultra-low card (k<=4, small range) leaves for counting residual.
 * Wide low-card still takes FEW_WIDE. Closes few_k4_dense soft spot.
...
inline bool should_try_few_wide(const int64_t *a, size_t n) {
    ...
    // Dense ultra-low card (k<=4, small range) → leave for counting residual.
    // Wide low-card still takes FEW_WIDE. Closes few_k4_dense soft spot.
