...
inline bool try_identity_almost(int64_t *a, size_t n) {
    ...
    for (size_t i = 0; i < n; i += step) { if (a[i] < mn) mn = a[i]; if (a[i] > mx) mx = a[i]; }
    ...
    for (size_t i = 0; i < early; ++i) if (a[i] != mn + (int64_t)i) ++early_mis;
    ...
    std::sort(vals, vals + d); std::sort(idx, idx + d);
    for (size_t k = 0; k < d; ++k) if (vals[k] != mn + (int64_t)idx[k]) { std::free(idx); std::free(vals); return false; }
    for (size_t k = 0; k < d; ++k) a[idx[k]] = vals[k];
    std::free(idx); std::free(vals); return true;
}

inline bool try_counting(int64_t *a, size_t n) {
    if (n < 2) return true;
    size_t S = n < 4096 ? n : 4096; size_t st = n / S; if (st < 1) st = 1;
    int64_t sample_vals[4096]; size_t ns = 0; int64_t smin = a[0], smax = a[0];
    for (size_t i = 0; i < n && ns < S; i += st) { int64_t v = a[i]; sample_vals[ns++] = v; if (v < smin) smin = v; if (v > smax) smax = v; }
    uint64_t srange = (uint64_t)(smax - smin);
    if (srange >= (1ull << 20) || srange >= (uint64_t)(n * 3 / 4)) return false;
    std::sort(sample_vals, sample_vals + ns);
    size_t sample_u = 1; for (size_t i = 1; i < ns; ++i) if (sample_vals[i] != sample_vals[i - 1]) ++sample_u;
    if (sample_u > 4096 || (sample_u > 1024 && srange > 262144ull)) return false;
    ...
    size_t p = 0; for (uint64_t v = 0; v <= range; ++v) for (size_t c = cnt[v]; c; --c) a[p++] = (int64_t)v + amin;
...
inline bool try_push_middle(int64_t *a, size_t n) {
    ...
    if (mid_n <= 64) { for (size_t i = mid_lo + 1; i < mid_hi; ++i) { int64_t key = a[i]; size_t j = i; while (j > mid_lo && a[j - 1] > key) { a[j] = a[j - 1]; --j; } a[j] = key; } }
    else residual_he::residual_he_msd_i64(a + mid_lo, mid_n);
    int64_t *tmp = (int64_t *)std::malloc(n * sizeof(int64_t)); if (!tmp) return false;
    size_t i = 0, j = mid_lo, k = mid_hi, p = 0;
    while (i < mid_lo && j < mid_hi && k < n) { int64_t x = a[i], y = a[j], z = a[k]; if (x <= y && x <= z) { tmp[p++] = x; ++i; } else if (y <= z) { tmp[p++] = y; ++j; } else { tmp[p++] = z; ++k; } }
    ...
    while (i < mid_lo) tmp[p++] = a[i++]; while (j < mid_hi) tmp[p++] = a[j++]; while (k < n) tmp[p++] = a[k++];
    std::memcpy(a, tmp, n * sizeof(int64_t)); std::free(tmp);
    for (size_t t = 1; t < n; ++t) if (a[t] < a[t - 1]) return false; return true;
}

inline bool try_reverse_runs(int64_t *a, size_t n) {
    if (n < 32) return false;
    size_t S = n < 8192 ? n - 1 : 8192; size_t st = (n - 1) / S; if (st < 1) st = 1;
    size_t sample_inv = 0, sample_desc_runs = 0, checked = 0; bool in_desc = false;
    for (size_t i = 0; i + 1 < n && checked < S; i += st, ++checked) {
        if (a[i] > a[i + 1]) { ++sample_inv; if (!in_desc) { ++sample_desc_runs; in_desc = true; } } else in_desc = false;
    }
    if (sample_desc_runs < 2 || sample_inv < 8) return false;
    if (checked > 0 && sample_inv * 2 > checked) return false;
    size_t i = 0, n_rev = 0;
    while (i + 1 < n) {
        if (a[i] > a[i + 1]) { size_t j = i + 1; while (j + 1 < n && a[j] > a[j + 1]) ++j; if (j - i + 1 >= 4) { std::reverse(a + i, a + j + 1); ++n_rev; } i = j + 1; } else ++i;
    }
    if (n_rev == 0) return false;
    constexpr size_t MAX_RUNS = 128; size_t run_end[MAX_RUNS]; int nr = 0;
    for (size_t k = 1; k < n; ++k) { if (a[k] < a[k - 1]) { if (nr >= (int)MAX_RUNS - 1) return false; run_end[nr++] = k; } }
    run_end[nr++] = n; if (nr == 1) return true; if (nr > 32) return false;
    int64_t *tmp = (int64_t *)std::malloc(n * sizeof(int64_t)); if (!tmp) return false;
    size_t ends_buf[MAX_RUNS]; for (int r = 0; r < nr; ++r) ends_buf[r] = run_end[r];
    int nends = nr; int64_t *src = a, *dst = tmp;
    while (nends > 1) {
        size_t new_ends[MAX_RUNS]; int nnew = 0, idx = 0; size_t cursor = 0;
        while (idx < nends) {
            if (idx + 1 >= nends) { std::memcpy(dst + cursor, src + cursor, (ends_buf[idx] - cursor) * sizeof(int64_t)); new_ends[nnew++] = ends_buf[idx]; break; }
            size_t s1 = cursor, e1 = ends_buf[idx], s2 = ends_buf[idx], e2 = ends_buf[idx + 1];
            size_t p = s1, i1 = s1, i2 = s2;
            while (i1 < e1 && i2 < e2) { if (src[i1] <= src[i2]) dst[p++] = src[i1++]; else dst[p++] = src[i2++]; }
            while (i1 < e1) dst[p++] = src[i1++]; while (i2 < e2) dst[p++] = src[i2++];
            new_ends[nnew++] = e2; cursor = e2; idx += 2;
        }
        for (int r = 0; r < nnew; ++r) ends_buf[r] = new_ends[r]; nends = nnew;
        int64_t *t = src; src = dst; dst = t;
    }
    if (src != a) std::memcpy(a, src, n * sizeof(int64_t)); std::free(tmp);
    for (size_t k = 1; k < n; ++k) if (a[k] < a[k - 1]) return false; return true;
}

inline int sort_i64(int64_t *a, size_t n) {
    if (n < 2) return 0;
    { const size_t S = n < 128 ? n : 128; size_t st = n / S; if (st < 1) st = 1; int64_t v0 = a[0]; bool maybe = true;
    ...
    if (n >= 64 && residual_few_wide::should_try_few_wide(a, n)) if (residual_few_wide::residual_few_wide_i64(a, n)) return 0;
    ...
    { size_t runs = 1; int dir = 0;
      for (size_t i = 1; i < n; ++i) {
        ...
        if (d == 0) continue;
        if (dir == 0) dir = d; else if (d != dir) { ++runs; dir = d; if (runs > 64) break; }
      }
    ...
    { size_t prefix_end = 1; while (prefix_end < n && a[prefix_end] >= a[prefix_end - 1]) ++prefix_end;
      ...
      if ((prefix_end >= n / 5 && right_run >= n / 5 && mid_n <= n / 4) || (prefix_end < n / 20 && right_run >= n / 2 && mid_n <= n / 4))
    ...
    { size_t S = n < 1024 ? n : 1024; size_t st = n / S; if (st < 1) st = 1;
      uint8_t seen[256] = {}; size_t distinct = 0, samples = 0; int64_t amin = a[0], amax = a[0];
      for (size_t i = 0; i < n && samples < S; i += st, ++samples) {
        uint64_t u = (uint64_t)a[i] ^ (1ull << 63); uint8_t tb = (uint8_t)(u >> 56);
        if (!seen[tb]) { seen[tb] = 1; ++distinct; }
        if (a[i] < amin) amin = a[i]; if (a[i] > amax) amax = a[i];
      }
      uint64_t span = (uint64_t)(amax - amin);
      if ((distinct <= 64 && span > 64ull * (uint64_t)n) || (distinct <= 32 && span > 16ull * (uint64_t)n))
