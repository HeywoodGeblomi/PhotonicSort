...
inline bool residual_two_value(uint32_t *a, size_t n) {
    if (n < 2) return true;
    uint32_t v0 = a[0], v1 = v0;
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (;;) {
            if (!used[h]) {
                ...
                used[h] = 1; keys[h] = v; counts[h] = 1; ++k; break;
            }
            if (keys[h] == v) { ++counts[h]; break; }
            h = (h + 1) & (HCAP - 1);
        }
    }
    uint32_t uniq[KMAX]; size_t cnt[KMAX]; size_t u = 0;
    for (size_t h = 0; h < HCAP; ++h) {
        if (!used[h]) continue;
        uint32_t key = keys[h]; size_t c = counts[h]; size_t j = u;
        while (j > 0 && uniq[j - 1] > key) { uniq[j] = uniq[j - 1]; cnt[j] = cnt[j - 1]; --j; }
        uniq[j] = key; cnt[j] = c; ++u;
    }
    size_t p = 0;
    for (size_t r = 0; r < u; ++r) {
        uint32_t v = uniq[r]; size_t c = cnt[r];
        while (c >= 8) {
            a[p]=v;a[p+1]=v;a[p+2]=v;a[p+3]=v;a[p+4]=v;a[p+5]=v;a[p+6]=v;a[p+7]=v;
            p += 8; c -= 8;
        }
        while (c--) a[p++] = v;
    }
...
inline bool should_try_few_wide(const uint32_t *a, size_t n) {
    if (n < 2) return false;
    size_t S = n < 4096 ? n : 4096;
    size_t st = n / S; if (st < 1) st = 1;
    uint32_t sample_vals[4096]; size_t ns = 0;
    uint32_t smin = a[0], smax = a[0];
    for (size_t i = 0; i < n && ns < S; i += st) {
        uint32_t v = a[i]; sample_vals[ns++] = v;
