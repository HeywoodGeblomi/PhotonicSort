 ...
 * Insertion (ultra-low inv local jitter) + std::sort residual.
 * Island reject removed — consecutive_perm + push_middle run earlier.
...
inline bool should_try_low_disorder(const uint32_t *a, size_t n) {
    ...
    // Island-shape reject removed: consecutive_perm + push_middle run earlier in menu.
