/*
    pdqsort_residual.h - Pattern-defeating quicksort (PhotonicSort residual path).
    ...
    Altered for PhotonicSort residual path: namespace residual_pdq_detail / residual_pdqsort.
...
#include <iterator>
    ...
    #define RESIDUAL_PDQSORT_PREFER_MOVE(x) std::move(x)
#else
    #define RESIDUAL_PDQSORT_PREFER_MOVE(x) (x)
#endif

namespace residual_pdq_detail {
    enum {
    ...
    inline void insertion_sort(Iter begin, Iter end, Compare comp) {
        ...
        for (Iter cur = begin + 1; cur != end; ++cur) {
            ...
            Iter sift_1 = cur - 1;
            if (comp(*sift, *sift_1)) {
                T tmp = RESIDUAL_PDQSORT_PREFER_MOVE(*sift);
                do { *sift-- = RESIDUAL_PDQSORT_PREFER_MOVE(*sift_1); }
                while (sift != begin && comp(tmp, *--sift_1));
                *sift = RESIDUAL_PDQSORT_PREFER_MOVE(tmp);
            }
        }
    }
    ...
    inline std::pair<Iter, bool> partition_right(Iter begin, Iter end, Compare comp) {
        typedef typename std::iterator_traits<Iter>::value_type T;
        T pivot(RESIDUAL_PDQSORT_PREFER_MOVE(*begin));
        Iter first = begin;
        Iter last = end;
        ...
        *begin = RESIDUAL_PDQSORT_PREFER_MOVE(*pivot_pos);
        *pivot_pos = RESIDUAL_PDQSORT_PREFER_MOVE(pivot);
...
inline void residual_pdqsort(Iter begin, Iter end, Compare comp) {
    if (begin == end) return;
    residual_pdq_detail::pdqsort_loop<Iter, Compare,
        residual_pdq_detail::is_default_compare<
            typename std::decay<Compare>::type>::value>(
        begin, end, comp,
        residual_pdq_detail::log2(end - begin));
}

template<class Iter>
inline void residual_pdqsort(Iter begin, Iter end) {
    typedef typename std::iterator_traits<Iter>::value_type T;
    residual_pdqsort(begin, end, std::less<T>());
}
