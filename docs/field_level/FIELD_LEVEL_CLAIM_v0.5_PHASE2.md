...
**Phase 2 PR:** #188 (`charged-surface-phase2`, merge 1db1d966)  
**Date:** 2026-08-24  
**Change character:** Charged surface expansion only — residual decision path untouched  
**Gate result:** soft@1.20 = 0 · major = 0 · ok = 1 (both ISAs)  
**EXTERNAL-clean.** No χ. Empty flip history.

## Constraint lock (held)
...
|---------|-----------|------------------------------|
| `late_phase_shift_dense` | `gen_late_phase_shift_dense` | Late cut @0.80 + denser local swaps → dual_owned=false band |
| `near_tie_he_even_starvation` | `gen_near_tie_he_even_starvation` | Even-event complement of Phase 1 odd starvation → dual_owned=false band |

Both stress the pure dual-evidence dual_owned boundary. No pure residual lever required. Menu routes under existing DEFAULT dual residual.

## Full charged surface (hard-fail) — Phase 2
...
**— Phase 2 sealed. soft@1.20 still zero. —**
